...
# clear everything
rm(list = ls())
cname <- file.path("C:\\Users\\maeli\\Documents\\INSA\\4A\\Taltech\\data_mining\\TD12\\Texts2")
...
inspect(docs[1]) #some data about a loaded text - order 2
writeLines(as.character(docs[1])) #content of text 2
...
length(freq)
standard_dev <- sd(freq)
#freq <- freq[freq >  standard_dev]
...
findAssocs(dtm, c("bee" , "honey"), corlimit=0.85) #find correlations between words
...
wordcloud::wordcloud(rownames(freq), freq[,1], scale=c(4, .1), max.words=40, colors=brewer.pal(6, "Dark2")) #most frequently used 50 words

#plot words that occur at least 50 times
wordcloud::wordcloud(rownames(freq), freq[,1], scale=c(4, .1), min.freq=6, colors=brewer.pal(6, "Dark2"))  #words used at least 25 times or more
