...
source("C:\\Users\\maeli\\Documents\\INSA\\4A\\Taltech\\data_mining\\Homework2\\ML_distance.R")
#load("C:\\Users\\maeli\\Documents\\INSA\\4A\\Taltech\\data_mining\\Homework2\\data\\ML_dataset_2D_labels.RData")
#load("C:\\Users\\maeli\\Documents\\INSA\\4A\\Taltech\\data_mining\\TD5\\Clusters_labels-W5Practice.RData")
load("C:\\Users\\maeli\\Documents\\INSA\\4A\\Taltech\\data_mining\\Homework2\\data\\ML_data_knn.RData")
...
par(mfrow=c(1,2))
# Data we were given
plot(x[,1],x[,2], col=x[,3]+1, pch=20,xlim=c(-20,35),ylim=c(-20,35), main="Original clustering")
#Training data
plot(x_train[,1], x_train[,2], pch=20, col=x_train[,3]+1,xlim=c(-20,35),ylim=c(-20,35), main="Using KNN", sub="The point circled in black are the validation set",xlab="x[,1]", ylab="x[,2]")
par(new=TRUE)
#Validation data
plot(predicted[,1], predicted[,2], pch=21, col="black", bg=predicted[,3]+1,xlim=c(-20,35),ylim=c(-20,35), xlab="", ylab="")


### ---- Accuracy ---- ###
size_confu <- nrow(confusion_matrix)
cpt_correct <- 0
for(i in seq(size_confu))
{
  cpt_correct <- cpt_correct + confusion_matrix[i,i]
}
accuracy <- (cpt_correct / nrow(x_validation))*100
cat("Accurcy is : ", accuracy, "\n")